 ...
 */
public class GoldCoinActivity extends AppCompatActivity implements Button.OnClickListener, AdapterView.OnItemSelectedListener{
    double price;
    ...
    private class GetMetalPriceFromJSON extends AsyncTask<String, Void, String> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.goldcoin_calculator);
        findViews();

        ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this,
                R.array.goldcoins_materials, android.R.layout.simple_spinner_item);
    ...
    public void onClick(View v) {
        if ( v == btnCalculate ) {
            ...
            if((spinMaterial.getSelectedItem().toString()).equals("Krugerrand, wholesale")){
                new GetMetalPriceFromJSON().execute(
                        "https://www.quandl.com/api/v1/datasets/WSJ/AU_ZAR.json?api_key=ATJdzsMvokeryKVx_cnz");
            }
            if((spinMaterial.getSelectedItem().toString()).equals("Maple Leaf, troy oz.")){
                new GetMetalPriceFromJSON().execute(
                        "https://www.quandl.com/api/v1/datasets/WSJ/AU_MPL.json?api_key=ATJdzsMvokeryKVx_cnz");
            }
            if((spinMaterial.getSelectedItem().toString()).equals("American Eagle, troy oz.")){
                new GetMetalPriceFromJSON().execute(
                        "https://www.quandl.com/api/v1/datasets/WSJ/AU_EGL.json?api_key=ATJdzsMvokeryKVx_cnz");
            }
